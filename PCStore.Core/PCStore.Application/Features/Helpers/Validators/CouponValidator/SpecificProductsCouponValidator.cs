...
{
    public class SpecificProductsCouponValidator(ProjectDbContext context) : ICouponValidator
    {
        public async Task<CouponValidatorResult> IsValid(string userId, Coupon coupon, List<CouponValidatorCommand> products)
        {
            ...
            var validProductIds = await context.CouponProducts
            ...
            var validProductsTotalCost = products.Where(x => validProductIds.Contains(x.ProductId)).Sum(x => x.ProductPrice * x.ItemCount);
