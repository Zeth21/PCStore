...
namespace PCStore.Application.Features.Helpers.Validators.CouponValidator
{
    public class CategoryCouponValidator(ProjectDbContext context) : ICouponValidator
    {
        public async Task<CouponValidatorResult> IsValid(string userId,Coupon coupon, List<CouponValidatorCommand> products)
        {
            ...
            var validProductsTotalCost = products.Where(x => validCategoryIds.Contains(x.ProductCategoryId)).Sum(x => x.ProductPrice * x.ItemCount);
